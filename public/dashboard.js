document.addEventListener('DOMContentLoaded', () => {
    ...
    const DOMElements = {
        ...
        upNextList: document.getElementById('up-next-list'), activityFeed: document.getElementById('activity-feed'), chartCanvas: document.getElementById('pipeline-chart'),
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const activityIcons = { 'created': '➕', 'started': '📞', 'complete': '✔', 'Deleted': '🗑️', 'Analyzed': '🧠' };
            const getIcon = (msg) => { for (const key in activityIcons) { if (msg.includes(key)) return activityIcons[key]; } return '🔔'; };
            DOMElements.activityFeed.innerHTML = data.activities.map(act => `<div class="activity-item"> <div class="icon">${getIcon(act.message)}</div> <div> <p>${act.message}</p> <p class="time">${dayjs(act.timestamp).fromNow()}</p> </div> </div>`).join('') || '<p class="text-secondary">No recent activity.</p>';
            const chartData = { labels: ['Awaiting Call', 'In Progress', 'Completed'], datasets: [{ data: [data.stats.awaiting_call_count, data.stats.in_progress_count, data.stats.completed_count], backgroundColor: ['#EAB308', '#3B82F6', '#22C55E'], borderWidth: 0 }] };
            if (pipelineChart) { pipelineChart.destroy(); }
            pipelineChart = new Chart(DOMElements.chartCanvas, { type: 'doughnut', data: chartData, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } } } });
    ...
    const renderPagination = (totalPages, activePage, container, pageType) => { container.innerHTML = ''; if (totalPages <= 1) return; for (let i = 1; i <= totalPages; i++) { const pageBtn = document.createElement('button'); pageBtn.className = `page-btn ${i === activePage ? 'active' : ''}`; pageBtn.textContent = i; pageBtn.dataset.page = i; pageBtn.dataset.pageType = pageType; container.appendChild(pageBtn); } };
    ...
    const renderDetails = (data) => {
        ...
        DOMElements.detailsContent.innerHTML = `
            <div class="details-header"><div class="avatar" style="background-color: ${bgColor};">${initials}</div><div><h2>${data.candidate_name}</h2><p>${data.job_position}</p></div></div>
            <div class="details-section"><h4>Key Skills</h4><div class="skill-tags-container">${skillTagsHtml || '<p class="text-secondary">No specific skills listed.</p>'}</div></div>
            <div class="details-summary-grid"><div class="summary-card"><div class="title">Overall AI Score</div><div class="value">${data.score || '--'}/100</div></div><div class="summary-card"><div class="title">AI Recommendation</div><div class="value recommendation status-badge ${getRecommendationClass(data.recommendation)}">${data.recommendation || '--'}</div></div></div>
            <div class="details-section"><h4>Detailed Analysis</h4><div class="detailed-score-grid">
                <div class="summary-card"><div class="title">Communication</div><div class="value">${data.comm_score || 'N/A'}/10</div></div>
                <div class="summary-card"><div class="title">Technical Knowledge</div><div class="value">${data.tech_score || 'N/A'}/10</div></div>
                <div class="summary-card"><div class="title">Job Relevance</div><div class="value">${data.relevance_score || 'N/A'}/10</div></div>
            </div></div>
            ${recordingHtml}
            <div class="details-section"><h4>AI Assessment</h4><p>${data.assessment || 'N/A'}</p></div>`;
    };
    ...
    DOMElements.talentPoolList.addEventListener('click', (e) => { const row = e.target.closest('tr'); if(row) showToast('This feature is for viewing past candidates.', 'info'); });
    ...
    [DOMElements.paginationControls, DOMElements.talentPoolPagination].forEach(p => p.addEventListener('click', (e) => { const btn = e.target.closest('.page-btn'); if (btn && !btn.classList.contains('active')) { if (btn.dataset.pageType === 'interviewPage') { state.currentPage = parseInt(btn.dataset.page); fetchAndRenderInterviews(); } else { state.talentPoolPage = parseInt(btn.dataset.page); fetchTalentPool(); } } }));
